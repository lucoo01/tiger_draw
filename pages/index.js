...
export default function Home() {
  ...
  // 简单处理跨域
  const transfn = async (q='') => {
	  
	  var url = 'https://替换成你自己的翻译接口网址/baidutrans?q=' + q + '&t=' + Math.floor(Math.random() * 9001) + 1000;
  ...
  const handleSubmit = async (e) => {
	...
	window.promptq = prompt;
	const q = prompt;

	const res = await transfn(q);
